...
namespace patches {
	namespace {
		...
		inline HWND WINAPI create_window_ex_w_stub(DWORD dw_ex_style, LPCWSTR lp_class_name, LPCWSTR lp_window_name, DWORD dw_style, int x, int y, int n_width, int n_height, HWND hwnd_parent, HMENU h_menu, HINSTANCE h_instance, LPVOID lp_param) {
			LPCWSTR lp_window_name_patched = lp_window_name;

			if (lp_class_name && ((uintptr_t)lp_class_name & 0xFFFF0000)) {
				if (wcscmp(lp_class_name, L"S1") == 0 || wcsstr(lp_window_name, L"Advanced Warfare") != nullptr) {
					lp_window_name_patched = L"s1s by Lifix Luxploit and Spet";
				}
			}		

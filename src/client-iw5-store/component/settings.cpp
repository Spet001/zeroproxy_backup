...
namespace settings {
	namespace {
		...
		std::string get_ini_path() {
			...
			return std::string(buffer) + "\\iw5s.ini";
		}
		...
		void load_ini(const std::string& path) {
			...
			LOG("Component/Settings", INFO, "Loaded iw5s.ini");
		}
	...
	DWORD WINAPI settings_thread(LPVOID) {
		while (true) {
			check_settings();
			Sleep(1000);
		}
	...
	void init() {
		check_settings();
		CreateThread(nullptr, 0, settings_thread, nullptr, 0, nullptr);
	}
}
